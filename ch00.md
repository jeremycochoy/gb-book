...
#### Status register
Status registers contains two kind of values.
First, it contains values that allow to control the processor.
An example is the enabling and disabling of interupts, controled
...
For example, the instruction `EI` set the interupt flag to on.
The second type are used by some instructions like conditional jumps.

### The memory
The memory is an other central part of the hardware.
The cpu comunicate with the memory through the memory BUS
(from the latine word omnibus).
The memory bus is a set of conductors (usually directly drawn on the circuit)
that allow the comminication between the memory chips and the CPU.
Retriving a specific byte is done by sending a number called the address
to the memory chips.
The memory allow storing the instruction executed by the processor,
but also the graphisms and divers data of the games.
